...
class Video_Stitching(QRunnable):
    ...
    def run(self):
        ...
        self.process_videos(self.video1_path, self.video2_path, self.output_path,self.signals)
        ##time.sleep(20)
    ...
    def compute_homography_matrix(self,frame1, frame2):
    ...
    def stitch_videos(self,video1_frames, video2_frames):
        ...
        for i, (frame1, frame2) in enumerate(zip(video1_frames, video2_frames)):
            frame1 = self.preprocess_image(frame1)
            frame2 = self.preprocess_image(frame2)

            if i == 0:
                # Compute the homography matrix from the first frame
                homography_matrix = self.compute_homography_matrix(frame1, frame2)
    ...
    def resize_frames(self,frames, target_size):
        resized_frames = []
        for frame in frames:
    ...
    def save_video(self,frames, output_path, frame_rate):
        if not frames:
        ...
        for i, frame in enumerate(frames):
            if frame is None or frame.size == 0:
                print(f"Skipping empty or None frame {i}.")
                continue
    ...
    def process_videos(self,video1_path, video2_path, output_path,signal):
        try:
            video1_frames = self.read_video_frames(video1_path)
            video2_frames = self.read_video_frames(video2_path)
            ...
            stitched_frames = self.stitch_videos(video1_frames, video2_frames)
            
            if stitched_frames:
                height, width = stitched_frames[0].shape[:2]
                resized_frames = self.resize_frames(stitched_frames, (width, height))
                start_time = time.time()
                self.save_video(resized_frames, output_path, frame_rate1)
                end_time = time.time()
                ...
                stitched = False
                print("No stitched frames available.")
                
            signal.stitching_complete.emit()
